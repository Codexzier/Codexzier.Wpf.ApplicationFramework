...
namespace WpfAppTemplateForNuget.Converters
{
    public class NormalizeTextConverter : IValueConverter
    {
        ...
        {
            if (value is string str && this.TryNormalizeText(str, out var result)) return result;
        ...
        private bool TryNormalizeText(string value, out string result)
        {
            ...
            if (!this._specializeCharReplaceAlternate.Keys.Any(a => value.Contains(a))) return false;

            foreach (var kvp in this._specializeCharReplaceAlternate) value = value.Replace(kvp.Key, kvp.Value);
            result = value;
